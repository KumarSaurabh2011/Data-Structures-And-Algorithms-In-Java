...
def scrapping_jobs(quarter_link, driver, scrip_code, scrip_name):
    ...
    promoter_link = driver.find_element(by=By.XPATH,value='//table[@class="tbmain"]/tbody/tr[1]/td/a')
    ...
    final_df = temp_promoter_df[['Scrip_code', 'Company', 'Quarter', 'Nationality', 'Category',
                                 'Category of shareholder', 'Nos. of shareholders', 'Total nos. shares held','Shareholding % calculated as per SCRR, 1957 As a % of (A+B+C2)']]
    try:
    ...
    time.sleep(get_random_wait(initial_limit=2, upper_limit=5))
    public_link = driver.find_element(by=By.XPATH,value='//table[@class="tbmain"]/tbody/tr[2]/td/a')
...
if __name__ == '__main__':
    ...
    for scrip_name, scrip_code in zip(scrips, codes):
        ...
        input_security = driver.find_element(by=By.XPATH,value='//input[@class="textbox2"]')
        time.sleep(get_random_wait(initial_limit=2, upper_limit=5))
        input_security.click()
        time.sleep(get_random_wait(initial_limit=2, upper_limit=5))
        input_security.send_keys(scrip_code)
        time.sleep(get_random_wait(initial_limit=2, upper_limit=5))
        select_security = driver.find_element(by=By.XPATH,value='//div[@id="ajax_response_smart"]/ul/li')
        time.sleep(get_random_wait(initial_limit=2, upper_limit=5))
        select_security.click()
        time.sleep(get_random_wait(initial_limit=2, upper_limit=5))
        period = driver.find_element(by=By.XPATH, value='//option[text()="Last 1 year"]')
        time.sleep(get_random_wait(initial_limit=2, upper_limit=5))
        period.click()
        time.sleep(get_random_wait(initial_limit=2, upper_limit=5))
        submit = driver.find_element(by=By.XPATH, value='//input[@value="Submit"]')
        time.sleep(get_random_wait(initial_limit=2, upper_limit=5))
        submit.click()
        time.sleep(get_random_wait(initial_limit=2, upper_limit=5))
        ...
        for i in range(1, 7):
            i = i+1
            try:
                quarter_link = driver.find_element(by=By.XPATH, value='//table[@class="mGrid"]/tbody/tr[{}]/td[4]/a'.format(i))
                time.sleep(get_random_wait(initial_limit=2, upper_limit=5))
